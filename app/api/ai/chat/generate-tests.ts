  ...
  try {
    if (!GROQ_API_KEY) {
      return res.status(500).json({
        error: "GROQ_API_KEY орнатылмаған. Vercel Environment Variables-қа GROQ_API_KEY қосыңыз.",
      });
    }

    const { content, count = 10, model = "llama-3.3-70b-versatile" } = req.body;

    if (!content || content.length < 50) {
      return res.status(400).json({ error: "Мәтін 50 таңбадан кем болмауы керек" });
    }
