// AITestGenerator.tsx — толық түзетілген нұсқа
...
function getFallbackQuestions(content: string): any[] {
  ...
  if (
    lower.includes("фотосинтез") ||
    lower.includes("хлоропласт") ||
    lower.includes("хлорофилл")
  ) {
    questions.push(
      {
        id: "f1",
        text: "Фотосинтез процесінде не түзіледі?",
        options: [
          "Глюкоза және оттегі",
          "Су және CO₂",
          "Азот",
      ...
      {
        id: "f2",
        text: "Фотосинтез қай органеллада жүреді?",
        options: ["Митохондрия", "Хлоропласт", "Рибосома", "Лизосома"],
        correctAnswer: 1,
        explanation: "Хлоропластта хлорофилл бар",
      },
      {
        id: "f3",
        text: "Фотосинтездің жарық фазасында түзіледі:",
        options: ["АТП және NADPH", "Глюкоза", "CO₂", "Су"],
        correctAnswer: 0,
        explanation: "Жарық реакцияларында АТП және NADPH",
      }
    );
  }

  if (
    lower.includes("жасуша") ||
    lower.includes("клетка") ||
    lower.includes("митохондрия")
  ) {
    questions.push(
      {
        id: "c1",
        text: "Жасушаның энергетикалық станциясы қайсы?",
        options: ["Ядро", "Митохондрия", "Рибосома", "Гольджи"],
        correctAnswer: 1,
        explanation: "Митохондрияда АТФ түзіледі",
      },
      {
        id: "c2",
        text: "Прокариоттарда жоқ органелла:",
        options: ["Рибосома", "Плазмида", "Ядро", "Цитоплазма"],
        correctAnswer: 2,
        explanation: "Прокариоттарда ядро жоқ",
      }
    );
  }
  ...
  if (
    lower.includes("днк") ||
    lower.includes("генетик") ||
    lower.includes("репликац")
  ) {
    questions.push(
      {
        id: "d1",
        text: "ДНҚ құрамында қандай азотты негіздер бар?",
        options: [
        ...
        correctAnswer: 0,
        explanation: "A-T, G-C. Урацил тек РНҚ-да",
      },
      {
        id: "d2",
        text: "ДНҚ репликациясы қай фазада жүреді?",
        options: ["G1", "S", "G2", "M"],
        correctAnswer: 1,
        explanation: "S-фазасы — синтез",
      }
    );
  }
  ...
  if (questions.length === 0) {
    questions.push(
      {
        id: "g1",
        text: "Берілген мәтін бойынша негізгі ұғым не?",
        options: [
          content.slice(0, 30) + "...",
          "Жасуша биологиясы",
          "Генетика",
        ...
        correctAnswer: 0,
        explanation: "Мәтін негізгі идеясы",
      },
      {
        id: "g2",
        text: "ОЗП биология пәнінде осы тақырып қандай үлесті алады?",
        options: ["30%", "25%", "15%", "10%"],
        correctAnswer: 2,
        explanation: "Тақырыпқа байланысты айтылады",
      }
    );
  }
...
export default function AITestGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setOnline(false);
      setError(
        "⚠️ Сервер offline — локалды сұрақтар берілді. GROQ_API_KEY тексеріңіз."
      );
    } finally {
      setLoading(false);
    }
            ...
            >
              {source === "text" ? (
                <div>
                  <label className="block text-xs text-[#64748b] mb-2">
                    Биология мәтінін қойыңыз
                  </label>
                  <textarea
                    value={text}
                    onChange={(e) => setText(e.target.value)}
                    placeholder="Мысалы: Фотосинтез — жарық энергиясын..."
                    rows={10}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  >
                    {["Негізгі", "Тереңдетілген", "Толық", "Жаңа"].map(
                      (c) => (
              ...
              <p className="text-[10px] text-[#475569] text-center">
                🤖 Groq API + Fallback • Сервер offline = локалды сұрақтар
              </p>
            </div>
          </div>
        ) : (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={handleSave}
                  className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#10b981] to-[#059669] text-white text-sm font-semibold shadow-lg shadow-[#10b981]/20 hover:shadow-[#10b981]/30 flex items-center gap-2 transition-all"
                >
