// ============================================
// 🤖📄 AI ТЕСТ ГЕНЕРАТОРЫ — НАҚТЫ GROQ API
// ============================================
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!data.questions || data.questions.length === 0) {
        setError("Сұрақтар генерацияланбады. Мәтінді толығырақ жазыңыз."); setLoading(false); return;
      }
      const test = {
      ...
      setError(err.message || "Серверге қосылу мүмкін болмады"); setOnline(false);
      ...
      <nav className="sticky top-0 z-50 bg-[#0f172a]/85 backdrop-blur-xl border-b border-white/[0.06]">
        <div className="max-w-[1200px] mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <h1 className="font-bold text-lg bg-gradient-to-r from-[#a855f7] to-[#3b82f6] bg-clip-text text-transparent flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-[#a855f7]" />AI Тест Генераторы</h1>
            ...
            <div ref={cardRef} style={cardStyle} className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-6 space-y-4">
              {source === "text" ? (
                <div>
                  ...
                  <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Мысалы: Фотосинтез — жарық энергиясын химиялық энергияға айналдыру процесі..." rows={10} className="w-full bg-[#0f172a]/80 border border-white/[0.12] rounded-xl px-4 py-3 text-sm text-[#f1f5f9] placeholder-[#475569] outline-none focus:border-[#a855f7] focus:shadow-[0_0_0_3px_rgba(168,85,247,0.15)] resize-none" />
              ...
              {error && <div className="flex items-center gap-2 text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3"><AlertCircle className="w-4 h-4 flex-shrink-0" />{error}</div>}
              ...
              <p className="text-[10px] text-[#475569] text-center">🤖 Groq AI (llama-3.3-70b) арқылы • Backend API</p>
            </div>
          </div>
        ) : (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-[#e2e8f0]">{generated.title} — {generated.questions.length} сұрақ</h2>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                        {q.options.map((opt: string, j: number) => (
                          <div key={j} className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm ${j === q.correctAnswer ? "bg-[#10b981]/10 border border-[#10b981]/30 text-[#6ee7b7]" : "bg-white/[0.03] border border-white/[0.06] text-[#94a3b8]"}`}>
                            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${j === q.correctAnswer ? "bg-[#10b981] text-white" : "bg-white/[0.08] text-[#64748b]"}`}>{String.fromCharCode(65 + j)}</span>{opt}{j === q.correctAnswer && <Check className="w-3.5 h-3.5 ml-auto text-[#10b981]" />}
                          </div>
                        ))}
                      </div>
