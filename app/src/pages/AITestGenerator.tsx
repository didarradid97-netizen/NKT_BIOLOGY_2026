// ============================================
// 🤖📄 AI ТЕСТ ГЕНЕРАТОРЫ — GROQ API + FALLBACK
// ============================================
...
function getFallbackQuestions(content: string): any[] {
  ...
  if (lower.includes("фотосинтез") || lower.includes("хлоропласт")) {
    questions.push(
      ...
      { id: "f3", text: "Фотосинтездің жарық фазасында түзіледі:", options: ["АТП және NADPH", "Глюкоза", "CO₂", "Су"], correctAnswer: 0, explanation: "Жарық реакцияларында АТП және NADPH" },
    );
  }
  if (lower.includes("жасуша") || lower.includes("клетка") || lower.includes("митохондрия")) {
    questions.push(
      ...
      { id: "c2", text: "Прокариоттарда жоқ органелла:", options: ["Рибосома", "Плазмида", "Ядро", "Цитоплазма"], correctAnswer: 2, explanation: "Прокариоттарда ядро жоқ" },
    );
  }
  if (lower.includes("днк") || lower.includes("генетик") || lower.includes("репликац")) {
    questions.push(
      ...
      { id: "d2", text: "ДНҚ репликациясы қай фазада жүреді?", options: ["G1", "S", "G2", "M"], correctAnswer: 1, explanation: "S-фазасы — синтез" },
    );
  }
  if (questions.length === 0) {
    questions.push(
      { id: "g1", text: "Берілген мәтін бойынша негізгі ұғым не?", options: [content.slice(0, 30) + "...", "Жасуша биологиясы", "Генетика", "Экология"], correctAnswer: 0, explanation: "Мәтін негізгі идеясы" },
      { id: "g2", text: "ОЗП биология пәнінде осы тақырып қандай үлесті алады?", options: ["30%", "25%", "15%", "10%"], correctAnswer: 2, explanation: "Тақырыпқа байланысты айтылады" },
    );
  }
  return questions;
}
...
export default function AITestGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    if (source === "text") {
      ...
      catch (e) { setError("Файлды оқу қатесі"); return; }
    }
    setLoading(true);
    try {
      ...
      setError("⚠️ Сервер offline — локалды сұрақтар берілді. GROQ_APIKEY тексеріңіз.");
        ...
        {!generated ? (
          <div className="space-y-6">
            ...
            <div ref={cardRef} style={cardStyle} className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-6 space-y-4">
              {source === "text" ? (
                <div><label className="block text-xs text-[#64748b] mb-2">Биология мәтінін қойыңыз</label><textarea value={text} onChange={e => setText(e.target.value)} placeholder="Мысалы: Фотосинтез — жарык энергиясын..." rows={10} className="w-full bg-[#0f172a]/80 border border-white/[0.12] rounded-xl px-4 py-3 text-sm text-[#f1f5f9] placeholder-[#475569] outline-none focus:border-[#a855f7] resize-none" /><p className="text-xs text-[#475569] mt-1">{text.length} таңба • кемінде 20</p></div>
              ) : (
                <div className="space-y-4"><div onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-white/[0.14] rounded-2xl p-8 text-center hover:border-[#a855f7]/40 hover:bg-white/[0.02] transition-all cursor-pointer"><Upload className="w-10 h-10 text-[#475569] mx-auto mb-3" /><p className="text-sm text-[#cbd5e1] font-medium">{file ? file.name : "TXT файлын жүктеңіз"}</p><p className="text-xs text-[#475569] mt-1">{file ? `${(file.size / 1024).toFixed(1)} KB` : "Макс. 5MB • тек TXT"}</p></div><input ref={fileRef} type="file" accept=".txt,text/plain" className="hidden" onChange={e => setFile(e.target.files?.[0] || null)} /></div>
              )}
              ...
              <button onClick={handleGenerate} disabled={loading} className="w-full py-3 rounded-xl bg-gradient-to-r from-[#a855f7] to-[#3b82f6] text-white font-semibold text-sm shadow-lg shadow-[#a855f7]/20 hover:shadow-[#a855f7]/30 flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-50">{loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Wand2 className="w-5 h-5" />}{load