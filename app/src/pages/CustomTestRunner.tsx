// ============================================
// ▶️ TEST RUNNER + AI ПОДСКАЗКА (Fallback)
// ============================================
...
import {
  ...
  CheckCircle, XCircle, RotateCcw, Home, Loader2, Sparkles, AlertTriangle
} from "lucide-react";
...
export default function CustomTestRunner() {
  ...
  const getHint = useCallback(async () => {
    ...
    try {
      ...
      setHint(data.hint || getLocalHint(q.text)); setOnline(true);
    } catch {
      setHint(getLocalHint(q.text)); setOnline(false);
    ...
    <div className="min-h-screen bg-[#0f172a] text-[#e2e8f0]">
      ...
      <div className="max-w-[900px] mx-auto px-4 py-6">
          ...
          <div className="space-y-3">
            {q.options.map((opt, idx) => (
              <button key={idx} onClick={() => handleAnswer(idx)} className={`w-full flex items-center gap-3 p-4 rounded-xl border text-left transition-all ${answers[current] === idx ? "bg-[#10b981]/10 border-[#10b981]/30" : "bg-white/[0.04] border-white/[0.08] hover:bg-white/[0.06]"}`}>
                <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${answers[current] === idx ? "bg-[#10b981] text-white" : "bg-white/[0.06] text-[#64748b]"}`}>{String.fromCharCode(65 + idx)}</span><span className="text-sm">{opt}</span>
              </button>
            ))}
          </div>
        </div>
        <div className="mb-6 bg-gradient-to-r from-[#f59e0b]/10 to-[#d97706]/5 border border-[#f59e0b]/20 rounded-2xl p-4">
          ...
          {!online && showHint && <div className="flex items-center gap-2 text-xs text-amber-400 mt-2"><AlertTriangle className="w-3 h-3" />Сервер offline — локалды подсказка</div>}
        </div>
