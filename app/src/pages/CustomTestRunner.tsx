// ============================================
// ▶️ TEST RUNNER + AI ПОДСКАЗКА — Backend API
// ============================================
...
export default function CustomTestRunner() {
  ...
  const getHint = useCallback(async () => {
    ...
    try {
      ...
      setErrorInfo("Подсказка API offline: " + err.message);
    ...
    <div className="min-h-screen bg-[#0f172a] text-[#e2e8f0]">
      ...
      <div className="max-w-[900px] mx-auto px-4 py-6">
        ...
        <div className="mb-6 bg-gradient-to-r from-[#f59e0b]/10 to-[#d97706]/5 border border-[#f59e0b]/20 rounded-2xl p-4">
          ...
          {showHint && (
            hintLoading ? <div className="flex items-center gap-2 text-sm text-[#94a3b8]"><Loader2 className="w-4 h-4 animate-spin text-[#fbbf24]" /> AI ойлауда...</div>
              : hint ? <div className="text-sm text-[#e2e8f0] leading-relaxed bg-white/[0.03] rounded-xl p-3">{hint}</div> : null
          )}
          {errorInfo && <div className="flex items-center gap-2 text-xs text-amber-400 mt-2"><AlertTriangle className="w-3 h-3" /> {errorInfo}</div>}
        </div>
