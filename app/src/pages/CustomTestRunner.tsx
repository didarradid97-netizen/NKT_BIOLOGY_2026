// CustomTestRunner.tsx — толық түзетілген нұсқа
...
export default function CustomTestRunner() {
  ...
  const getHint = useCallback(async () => {
    ...
    try {
      ...
      setHint(data.hint || getLocalHint(q.text));
      setOnline(true);
    } catch {
      ...
      <div className="min-h-screen bg-[#0f172a] text-[#e2e8f0]">
        <div className="max-w-[800px] mx-auto px-4 py-12">
          <div className="text-center mb-8">
            ...
            <p className="text-4xl font-extrabold bg-gradient-to-r from-[#34d399] to-[#3b82f6] bg-clip-text text-transparent">
              {correct}/{test.questions.length} ({pct}%)
            </p>
          </div>
          ...
          <div className="flex gap-3 justify-center">
            <button
              ...
              className="px-5 py-3 rounded-xl bg-white/[0.06] text-[#e2e8f0] font-medium flex items-center gap-2"
            >
            ...
            <button
              ...
              className="px-5 py-3 rounded-xl bg-gradient-to-r from-[#10b981] to-[#059669] text-white font-semibold flex items-center gap-2"
            >
      ...
      <div className="sticky top-0 z-50 bg-[#0f172a]/90 backdrop-blur-xl border-b border-white/[0.06]">
        <div className="max-w-[900px] mx-auto px-4 py-3 flex items-center justify-between">
          <button
            ...
            className="p-2 rounded-lg bg-white/[0.06] text-[#94a3b8] hover:text-white"
          >
          ...
          <div
            className={`flex items-center gap-1.5 text-sm font-mono ${
              timeLeft < 60 ? "text-red-400" : "text-[#e2e8f0]"
            }`}
          >
      ...
      <div className="max-w-[900px] mx-auto px-4 py-6">
        <div className="mb-6">
          <span className="text-sm text-[#94a3b8]">
            Сұрақ {current + 1} / {test.questions.length}
          </span>
          <h2 className="text-lg font-bold text-[#e2e8f0] mt-4 mb-6">
            {q.text}
          </h2>
          {q.image && (
            <img
              src={q.image}
              alt=""
              className="max-h-48 rounded-xl mb-4 border border-white/[0.08]"
            />
          )}
        ...
        <div className="mb-6 bg-gradient-to-r from-[#f59e0b]/10 to-[#d97706]/5 border border-[#f59e0b]/20 rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              <select
                value={hintType}
                onChange={(e) =>
                  setHintType(e.target.value as "mini" | "full")
                }
                className="bg-[#0f172a]/80 border border-white/[0.12] rounded-lg px-2 py-1 text-xs text-[#e2e8f0]"
              >
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-[#f59e0b]/15 text-[#fbbf24] text-xs font-medium flex items-center gap-1 hover:bg-[#f59e0b]/25 transition-all disabled:opacity-50"
              >
                {hintLoading ? (
          ...
          {showHint &&
            (hintLoading ? (
              ...
              <div className="text-sm text-[#e2e8f0] leading-relaxed bg-white/[0.03] rounded-xl p-3">
                {hint}
              </div>
            ) : null)}
          {!online && showHint && (
            <div className="flex items-center gap-2 text-xs text-amber-400 mt-2">
              ...
              Сервер offline — локалды подсказка
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between">
          <button
            ...
            disabled={current === 0}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/[0.06] text-[#e2e8f0] text-sm disabled:opacity-30 transition-all"
          >
          ...
          {current < test.questions.length - 1 ? (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#10b981] to-[#059669] text-white text-sm font-medium shadow-lg shadow-[#10b981]/20 transition-all"
            >
              Келесі
            ...
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#3b82f6] to-[#6366f1] text-white text-sm font-semibold shadow-lg shadow-[#3b82f6]/20 transition-all"
            >
              Аяқтау
              <CheckCircle className="w-4 h-4" />
            </button>
