// AITrainer.tsx — толық түзетілген нұсқа
...
const saveChatMessage = (msg: Message) => {
  ...
  h.push(msg);
  localStorage.setItem("nkt_chat_history", JSON.stringify(h.slice(-100)));
};
      ...
      <div className="max-w-[900px] mx-auto px-4 py-6 flex flex-col h-[calc(100vh-64px)]">
        {showWelcome && messages.length === 0 && (
          ...
          >
            ...
            <h1 className="text-2xl font-bold mb-2">
              AI Биология Жаттықтырушысы
            </h1>
            <p className="text-[#94a3b8] max-w-md mb-8">
              Groq AI (llama-3.3-70b) арқылы нақты жауап. Сервер offline болса —
              локалды жауап беремін.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-lg">
              {[
                ...
                "Мейоз бен митоздың айырмашылығы?",
              ].map((q) => (
                <button
                  key={q}
                  onClick={() => {
        ...
        {!online && (
          <div className="flex items-center gap-2 text-xs text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2 mb-3">
            ...
            Сервер offline. Fallback (локалды) жауап берілуде.
          </div>
        )}
        ...
        <div className="border-t border-white/[0.08] pt-4">
          <div className="flex gap-2 items-end">
            ...
            <button
              ...
              className="h-11 px-4 rounded-xl bg-gradient-to-r from-[#10b981] to-[#059669] text-white font-semibold text-sm shadow-lg disabled:opacity-40 transition-all flex items-center gap-2"
            >
              {loading ? (
          ...
          <p className="text-[10px] text-[#475569] mt-2 text-center">
            🤖 Groq API + Fallback • Сервер offline = локалды жауап
          </p>
        </div>
      </div>
    </div>
