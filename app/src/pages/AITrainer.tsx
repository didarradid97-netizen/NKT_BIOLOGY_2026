...
// ✅ Groq API тікелей фронтендтен (Vite)
...
export default function AITrainer() {
  ...
  const handleClear = () => {
    if (confirm("Барлық әңгімені жою керек пе?")) {
      clearChatHistory(); setMessages([]); setShowWelcome(true);
    }
  };
    ...
    <div className="min-h-screen bg-[#0f172a] text-[#e2e8f0]">
      <nav className="sticky top-0 z-50 bg-[#0f172a]/85 backdrop-blur-xl border-b border-white/[0.06]">
        <div className="max-w-[1200px] mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <button onClick={handleClear} className="p-2 rounded-lg bg-white/[0.06] hover:bg-red-500/15 text-[#94a3b8] hover:text-red-400 transition-all" title="Тазалау">
      ...
      <div className="max-w-[900px] mx-auto px-4 py-6 flex flex-col h-[calc(100vh-64px)]">
        {showWelcome && messages.length === 0 && (
          <div ref={cardRef} className="flex-1 flex flex-col items-center justify-center text-center" style={{ transform }}>
            ...
            <p className="text-[#94a3b8] max-w-md mb-8">
              Маған қазақша сұрақ қойыңыз — жасуша, генетика, эволюция, экология, физиология тақырыптарында көмектесемін.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-lg">
              {["Жасуша құрылысы қандай?", "Фотосинтез процесі қалай жүреді?", "ДНҚ репликациясы дегеніміз не?", "ОЗП-ға қалай дайындалу керек?"].map((q) => (
          ...
          <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-1">
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                ...
                <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${msg.role === "user" ? "bg-[#3b82f6]/20 border border-[#3b82f6]/30" : "bg-white/[0.05] border border-white/[0.08]"} text-[#e2e8f0]`}>
                  {msg.content}
                </div>
              </div>
            ))}
        ...
        <div className="border-t border-white/[0.08] pt-4">
          ...
          <p className="text-[10px] text-[#475569] mt-2 text-center">Groq AI · Llama 3.3 70B · Қазақша биология жаттықтырушысы</p>
        </div>
      </div>
    </div>
  );
