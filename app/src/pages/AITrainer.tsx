// ============================================
// 🤖 AI ЖАТТЫҚТЫРУШЫ — GROQ API + FALLBACK
// ============================================
...
const FALLBACK_RESPONSES: Record<string, string> = {
  ...
  "днк": "ДНҚ — екі тізбекті спираль. A-T, G-C. Репликация: S-фаза. Полимераза. Транскрипция: ДНҚ→иРНҚ. Трансляция: иРНҚ→ақуыз.",
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const assistantMsg: Message = {
          role: "assistant",
          content: fallback || "Серверге қосылу мүмкін болмады.",
      ...
      const assistantMsg: Message = {
        role: "assistant",
        content: fallback || "⚠️ Сервер offline. GROQ_API_KEY тексеріңіз.",
        timestamp: new Date().toISOString(),
      };
      setMessages(prev => [...prev, assistantMsg]); saveChatMessage(assistantMsg); setOnline(false);
      ...
      <div className="max-w-[900px] mx-auto px-4 py-6 flex flex-col h-[calc(100vh-64px)]">
        {showWelcome && messages.length === 0 && (
          <div ref={cardRef} className="flex-1 flex flex-col items-center justify-center text-center" style={{ transform }}>
            ...
            <div className="mt-6 flex items-center gap-2 text-xs text-[#475569]"><BookOpen className="w-3.5 h-3.5" />Groq AI • Fallback • Барлығы 9 тақырып</div>
          </div>
        )}
        {messages.length > 0 && (
          <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-1">
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === "user" ? "bg-gradient-to-br from-[#3b82f6] to-[#10b981]" : "bg-gradient-to-br from-[#10b981] to-[#059669]"}`}>{msg.role === "user" ? <User className="w-4 h-4 text-white" /> : <Bot className="w-4 h-4 text-white" />}</div>
        ...
        {!online && (
          <div className="flex items-center gap-2 text-xs text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2 mb-3">
            <AlertTriangle className="w-3.5 h-3.5" />Сервер offline. Fallback (локалды) жауап берілуде.
          </div>
        )}
        <div className="border-t border-white/[0.08] pt-4">
          <div className="flex gap-2 items-end">
            <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Қазақша сұрақ жазыңыз..." rows={1} className="flex-1 bg-[#0f172a]/80 border border-white/[0.12] rounded-xl px-4 py-3 text-sm text-[#f1f5f9] placeholder-[#64748b] outline-none focus:border-[#10b981] resize-none min-h-[44px] max-h-[120px]" />
            <button onClick={handleSend} disabled={!input.trim() || loading} className="h-11 px-4 rounded-xl bg-gradient-to-r from-[#10b981] to-[#059669] text-white font-semibold text-sm shadow-lg disabled:opacity-40 transition-all flex items-center gap-2">{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}<span className="hidden sm:inline">Жіберу</span></button>
          </div>
          <p className="text-[10px] text-[#475569] mt-2 text-center">🤖 Groq API + Fallback • Сервер offline = локалды жауап</p>
        </div>
      </div>
    </div>
  );
